...
MarkerControl.prototype.create = function (wrapper) {
	...
	buttonBack.setAttribute("class", "list-group-item");
	buttonBack.innerHTML = "Back to index";

	var addMarkers = document.createElement("button");
	addMarkers.setAttribute("class", "list-group-item");
	addMarkers.innerHTML = "Add markers";
	addMarkers.addEventListener("click", function (handler) {
		return function () {
	...
	listGroup.appendChild(addMarkers);
	listGroup.appendChild(buttonBack);
	...
	setTimeout(function () {
		...
		panelHeadings.forEach(function (heading) {
			heading.addEventListener('click', function () {
				let listGroup = heading.nextElementSibling;
