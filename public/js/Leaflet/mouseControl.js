...
MousePosControl.prototype.create = function(wrapper) {
	...
	var updatePos = function(ui) {
		return function(event) {
			...
			+ '<button type="button" class="btn btn-light">' + "X: " + Math.round(xzy[0]) + '</button>'
			+ '<button type="button" class="btn btn-light">' + "Z: " + Math.round(xzy[1]) + '</button>'
			+ '<button type="button" class="btn btn-light">' + "Y: " + Math.round(xzy[2]) + '</button>'
			+ "</div>";
		};
	}(this.ui);
